...
class AdminSystem:
    ...
    async def admin_edit_user(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not await self.bot.is_owner(update):
    ...
    async def admin_system_stats(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Базовая статистика
        cursor = await self.bot.conn.execute('SELECT COUNT(*) FROM users')
        total_users = (await cursor.fetchone())[0]
        
        cursor = await self.bot.conn.execute('SELECT SUM(balance) FROM users')
        total_coins = (await cursor.fetchone())[0] or 0
        
        cursor = await self.bot.conn.execute('SELECT AVG(balance) FROM users')
        avg_balance = (await cursor.fetchone())[0] or 0
        
        cursor = await self.bot.conn.execute('SELECT COUNT(*) FROM users WHERE balance > 1000')
    ...
    async def admin_user_search(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        cursor = await self.bot.conn.execute(query, params)
